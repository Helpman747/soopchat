...
# 채팅에 연결
async def connect_to_chat(bid, ssl_context):
    try:
        result = get_player_live(bid)  # bno 파라미터 제거
        if not result:
            return
...
async def main():
    bid = input("아프리카TV BJ 아이디를 입력해주세요: ")  # 예: devil0108
    ssl_context = create_ssl_context()
    await connect_to_chat(bid, ssl_context)
